...
class BaseModel:  # pylint: disable=too-few-public-methods,too-many-public-methods
    ...
    def find(cls, amount: int):
        """Return matching records relative to amount"""
        result = cls.where().limit(amount).fetch()
        return result

    @classmethod
    def find_or_fail(cls, amount: int):
        """Return matching records relative to amount, will throw if amount fetched is equal to 0"""
        result = cls.where().limit(amount).throw().fetch()
        return result
    ...
    def first_or_fail(cls, **kwargs):
        """Return the first matching record or None if no match is found."""
        result: Self = cls.where(**kwargs).limit(1).throw().fetch_one() # type: ignore
        return result
