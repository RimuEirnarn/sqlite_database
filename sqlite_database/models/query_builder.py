...
from .errors import EmptyModelError
...
class QueryBuilder(Generic[T]):
    ...
    def throw(self):
        """Set fail/throw flag to true so we return an error than None"""
        self._failing = True
        return self
    ...
    def fetch(self) -> list[T]:
        ...
        records: list[dict[str, Any]] = self._model.get_table().select(
            self._filters, limit=self._limit, offset=self._offset, order=self._order
        ) # type: ignore
        if len(records) == 0 and self._failing:
            raise EmptyModelError(f"No data is found for {self._model.__name__}")
        return [
    ...
    def fetch_one(self) -> T | None:
        ...
        if record is None and self._failing:
            raise EmptyModelError(f"No data is found for {self._model.__name__}")
        if not record:
            return None
